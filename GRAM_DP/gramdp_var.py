...
def gramdp_var(array, desired_privacy, iterations):
    r"""Compute the differentially private variance along the specified axis.
    ...
    using :class:`.LaplaceBoundedDomain` to satisfy differential privacy.
    ...
    * higher privacy leads to lower utility and vice versa.
    ...
    sensitivity = ((maximum - minimum) / len(array)) ** 2 * (len(array) - 1)
    true_var = np.var(array)
    DP_result = []
    std_se_error = []
    percentage_error = []
    ...
    for itr in range(iterations):
        ...
        dp_result = (true_var + noise)
        DP_result.append(dp_result)
        percentage_error.append(abs(((true_var - dp_result)/true_var)*100))
        std_se_error.append(abs((true_var - dp_result)/len(array)))
        
        dp_MSE_results.append(((true_var - dp_result)**2)/iterations)
        dp_scaled_error_results.append(abs(true_var - dp_result)/iterations)
        dp_percentage_error_results.append(np.square((true_var - dp_result)/true_var))
...
#print(gramdp_var(array=[10,23,45,76,8,2,34,65], desired_privacy='very_high', iterations=50))
