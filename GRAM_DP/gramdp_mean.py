...
# Gram-DP focus on bounded Differential Privacy for mean query.

def gramdp_mean(array, desired_privacy, iterations):
    r"""Compute the differentially private arithmetic mean along the specified axis.

    Returns the average of the array elements with differential privacy. Noise is added using np.Laplace` to satisfy differential
    ...
    * higher privacy leads to lower utility and vice versa.
    ...
    Returns
    -------
    mean : int or array of int
        Differentially private mean of an array.

    """
    maximum = np.max(array) * 1.01
    minimum = np.min(array) * 0.99
    sensitivity = (maximum - minimum)/len(array)
    ...
    for itr in range(iterations):
        ...
        percentage_error.append(abs(((true_mean - dp_result)/true_mean)*100))
        std_se_error.append(abs((true_mean - dp_result)/len(array)))
            
        dp_MSE_results.append(((true_mean - dp_result)**2)/iterations)
        dp_scaled_error_results.append(abs(true_mean - dp_result)/iterations)
        dp_percentage_error_results.append(np.square((true_mean - dp_result)/true_mean))
...
#print(gramdp_mean(array=[10,23,45,76,8,2,34,65], desired_privacy='very_high', iterations=50))
